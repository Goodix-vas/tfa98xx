...
enum Tfa98xx_Error tfa9875_tfa_status(struct tfa_device* tfa)
{
	...
	if (!tfa_get_bf_value(TFA9875_BF_UVDS, val))
		tfa_set_bf_volatile(tfa, (uint16_t)TFA9875_BF_UVDS, 1);
	if (!tfa_get_bf_value(TFA9875_BF_OVDS, val))
		tfa_set_bf_volatile(tfa, (uint16_t)TFA9875_BF_OVDS, 1);
	if (!tfa_get_bf_value(TFA9875_BF_OTDS, val))
		tfa_set_bf_volatile(tfa, (uint16_t)TFA9875_BF_OTDS, 1);
	if (tfa_get_bf_value(TFA9875_BF_OCDS, val))
		tfa_set_bf_volatile(tfa, (uint16_t)TFA9875_BF_OCDS, 1);
	if (tfa_get_bf_value(TFA9875_BF_NOCLK, val))
		tfa_set_bf_volatile(tfa, (uint16_t)TFA9875_BF_NOCLK, 1);
	/*
	...
	if (tfa_get_bf(tfa, TFA9875_BF_BODNOK))
	{
		...
		tfa_set_bf_volatile(tfa, (uint16_t)TFA9875_BF_BODNOK, 1);
	}
...
static enum Tfa98xx_Error tfa9861_specific(struct tfa_device *tfa)
{
	...
	case 0x1a61: /* Initial revision ID TFA9861 N1A1 */
		...
		/* ----- generated code end   ----- */
		break;
	case 0x2a61: /* Initial revision ID TFA9861 N1A2 */
        ...
        tfa_reg_write(tfa, 0x63, 0x805b); //POR=0x9062
        tfa_reg_write(tfa, 0x64, 0x020a); //POR=0x000b
        tfa_reg_write(tfa, 0x6c, 0xfb80); //POR=0xf800
        tfa_reg_write(tfa, 0x74, 0x60bc); //POR=0x6014
        tfa_reg_write(tfa, 0x75, 0x3daa); //POR=0x49e0
        tfa_reg_write(tfa, 0x7b, 0x006d); //POR=0x0045
        tfa_reg_write(tfa, 0xb1, 0x0000); //POR=0x000f
        ...
        /* ----- generated code end   ----- */
		break;
    case 0x3a61: /* Initial revision ID TFA9861 N1A3 */
        /* ----- generated code start(V6) ----- */
        /* -----  version 4 ----- */
        tfa_reg_write(tfa, 0x02, 0x0c78); //POR=0x0008
        tfa_reg_write(tfa, 0x04, 0x8740); //POR=0x0740
	...
	case 0x1a63: /* Initial revision ID TFA9863 N1A1 */
		...
		tfa_reg_write(tfa, 0xd9, 0x8000); //POR=0x0000
		break;
	case 0x2a63: /* Initial revision ID TFA9863 N1A2 */
		tfa_reg_write(tfa, 0x02, 0x0c78); //POR=0x0008
		tfa_reg_write(tfa, 0x04, 0x8740); //POR=0x0740
		tfa_reg_write(tfa, 0x07, 0x00c0); //POR=0x0048
		tfa_reg_write(tfa, 0x08, 0x00c2); //POR=0x009a
		...
		/* ----- generated code end   ----- */
		break;
    case 0x3a63: /* Initial revision ID TFA9863 N1A3 */
		tfa_reg_write(tfa, 0x02, 0x0c78); //POR=0x0008
		tfa_reg_write(tfa, 0x04, 0x8740); //POR=0x0740
		tfa_reg_write(tfa, 0x07, 0x00c0); //POR=0x0048
		tfa_reg_write(tfa, 0x08, 0x00c2); //POR=0x009a
		...
		tfa_reg_write(tfa, 0xb1, 0x0000); //POR=0x000f
		tfa_reg_write(tfa, 0xcc, 0xf200); //POR=0xf000
		tfa_reg_write(tfa, 0xd9, 0x8000); //POR=0
        break;
	default:
		pr_info("\nWarning: Optimal settings not found for device with revid = 0x%x \n", tfa->revid);
		break;
...
static enum Tfa98xx_Error tfa9867_specific(struct tfa_device *tfa)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	unsigned short value, xor, rc;
	unsigned short irqmask;
	...
	tfa_set_bf(tfa, TFA9867_BF_PWDN, 0);
	tfa_set_bf(tfa, TFA9867_BF_MANAOOSC, 0);
		
	rc = tfa_wait4manstate(tfa, TFA9867_BF_MANSTATE, 1, 50);
	if (rc < 0) {
		pr_err("Error, waiting powerdown leaving\n");
		return rc;
	}
	...
	switch (tfa->revid) {
	case 0x0a67: /* Initial revision ID TFA9867 N1A0 */
		/* ----- generated code start(V6) ----- */
		/* -----  version 28 ----- */
		tfa_reg_write(tfa, 0x00, 0xf241); //POR=0xf265
		tfa_reg_write(tfa, 0x08, 0x007a); //POR=0x00d2
		tfa_reg_write(tfa, 0x50, 0xc000); //POR=0x8000
		tfa_reg_write(tfa, 0x54, 0x20e0); //POR=0x00e0
		tfa_reg_write(tfa, 0x5a, 0x5f5e); //POR=0x36be
		...
		tfa_reg_write(tfa, 0x68, 0x0820); //POR=0x0c20
		tfa_reg_write(tfa, 0x69, 0x0119); //POR=0x0319
		tfa_reg_write(tfa, 0x74, 0x6014); //POR=0x4c14
	...
	case 0x102a67: /* Initial revision ID TFA9867 N2A2 */
        /* ----- generated code start(V6) ----- */
        /* -----  version 24 ----- */
        tfa_reg_write(tfa, 0x00, 0xf321); //POR=0xf361
        tfa_reg_write(tfa, 0x01, 0x0aa0); //POR=0x8aa0
        tfa_reg_write(tfa, 0x04, 0xc758); //POR=0xc340
        tfa_reg_write(tfa, 0x08, 0x009a); //POR=0x00d2
        tfa_reg_write(tfa, 0x50, 0xc000); //POR=0x8000
        tfa_reg_write(tfa, 0x53, 0x002e); //POR=0x0028
        tfa_reg_write(tfa, 0x54, 0xb4e0); //POR=0xa0e0
        tfa_reg_write(tfa, 0x58, 0x1001); //POR=0x0000
        tfa_reg_write(tfa, 0x5a, 0x5f40); //POR=0x36a0
        tfa_reg_write(tfa, 0x5b, 0x74e2); //POR=0x7329
        tfa_reg_write(tfa, 0x5c, 0xb02b); //POR=0xde96
        tfa_reg_write(tfa, 0x5f, 0x00a0); //POR=0x00c0
        tfa_reg_write(tfa, 0x60, 0x0040); //POR=0x4040
        tfa_reg_write(tfa, 0x62, 0x26c8); //POR=0x0582
        tfa_reg_write(tfa, 0x63, 0x80d4); //POR=0x0602
        tfa_reg_write(tfa, 0x64, 0x0207); //POR=0x020b
        tfa_reg_write(tfa, 0x66, 0x0fc0); //POR=0x4040
        tfa_reg_write(tfa, 0x67, 0x00e8); //POR=0x0602
        tfa_reg_write(tfa, 0x68, 0x0820); //POR=0x0c20
        tfa_reg_write(tfa, 0x69, 0x0119); //POR=0x0319
        tfa_reg_write(tfa, 0x74, 0x6094); //POR=0x4c14
        tfa_reg_write(tfa, 0x75, 0x15d1); //POR=0x49e0
        tfa_reg_write(tfa, 0x78, 0x0071); //POR=0x0061
        tfa_reg_write(tfa, 0x7b, 0x0fd5); //POR=0x0045
        tfa_reg_write(tfa, 0x7c, 0x10e3); //POR=0x1602
        tfa_reg_write(tfa, 0x7d, 0x0032); //POR=0x0039
        tfa_reg_write(tfa, 0xb1, 0x0008); //POR=0x0000
        tfa_reg_write(tfa, 0xda, 0x0823); //POR=0x0803
        /* ----- generated code end   ----- */
		break;
	default:
		pr_info("\nWarning: Optimal settings not found for device with revid = 0x%x \n", tfa->revid);
	...
	tfa_set_bf(tfa, TFA9867_BF_PWDN, 1); /* 1 = off */
	rc = tfa_wait4manstate(tfa, TFA9867_BF_MANSTATE, 0, 50);
	if (rc < 0)
	{
		pr_err("Timeout waiting for manstate 0\n");
		return rc;
	...
	/* we come from reset state so turn off osc */
	tfa_set_bf(tfa, TFA9867_BF_MANAOOSC, 1);

	/* select error interrupts */
	irqmask = (TFA_BF_MSK(TFA9867_BF_IEBSTOC) |
				TFA_BF_MSK(TFA9867_BF_IEOTDS) |
				TFA_BF_MSK(TFA9867_BF_IEOCPR) |
				TFA_BF_MSK(TFA9867_BF_IEUVDS) |
				TFA_BF_MSK(TFA9867_BF_IEBODNOK)|
				TFA_BF_MSK(TFA9867_BF_IECOOR)|
				TFA_BF_MSK(TFA9867_BF_IEOVDS));
...
static int tfa9867_set_bitfield(struct tfa_device* tfa, uint16_t bitfield, uint16_t value)
{
	if (((bitfield >> 8) & 0xff) == 0x10 || ((bitfield >> 8) & 0xff) == 0x13)
		return tfa_set_bf_volatile(tfa, (uint16_t)bitfield, value);
	else
...
static int tfa9867_set_swprofile(struct tfa_device *tfa, unsigned short new_value)
{
	int active_value = tfa_dev_get_swprof(tfa);
	...
	/* Set the new value in the hw register */
	tfa_set_bf_volatile(tfa, TFA9867_BF_SWPROFIL, new_value);

	return active_value;
}

static int tfa9867_get_swprofile(struct tfa_device *tfa)
{
	return tfa_get_bf(tfa, TFA9867_BF_SWPROFIL) - 1;
}

static int tfa9867_set_swvstep(struct tfa_device *tfa, unsigned short new_value)
{
	...
	/* Set the new value in the hw register */
	tfa_set_bf_volatile(tfa, TFA9867_BF_SWVSTEP, new_value);

	return new_value;
}

static int tfa9867_get_swvstep(struct tfa_device *tfa)
{
	return tfa_get_bf(tfa, TFA9867_BF_SWVSTEP) - 1;
}

/* tfa98xx_dsp_system_stable
*  return: *ready = 1 when clocks are stable to allow DSP subsystem access
*/
static enum Tfa98xx_Error tfa9867_dsp_system_stable(struct tfa_device *tfa, int *ready)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;

	/* check CLKS: ready if set */
	*ready = tfa_get_bf(tfa, TFA9867_BF_CLKS) == 1;
...
void tfa9867_ops(struct tfa_device_ops *ops)
{
	...
	ops->get_mtpb = NULL; /* no mtp, used as check for new efuse types */
	ops->tfa_init = tfa9867_specific;
	ops->set_swprof = tfa9867_set_swprofile;
	ops->get_swprof = tfa9867_get_swprofile;
	ops->set_swvstep = tfa9867_set_swvstep;
	ops->get_swvstep = tfa9867_get_swvstep;
	ops->dsp_system_stable = tfa9867_dsp_system_stable;
	ops->set_mute = tfa_set_mute_nodsp;
	ops->tfa_set_bitfield = tfa9867_set_bitfield;
}
...
static enum Tfa98xx_Error tfa986x_specific(struct tfa_device *tfa)
{
	...
	rc = tfa_wait4manstate(tfa, TFA986X_BF_MANSTATE, 1, 50);
	if (rc < 0) {
		pr_err("Error, waiting powerdown leaving\n");
		return rc;
	}
	...
	switch (tfa->revid) {
	case 0x3a64:/**TFA9864 N1A3**/
		/* ----- generated code start(V6) ----- */
		/* -----  version 2 ----- */
		tfa_reg_write(tfa, 0x00, 0xf241); //POR=0xf261
		tfa_reg_write(tfa, 0x02, 0x0628); //POR=0x0008
		tfa_reg_write(tfa, 0x50, 0xc000); //POR=0x8000
		tfa_reg_write(tfa, 0x5a, 0x5f4c); //POR=0x36be
		tfa_reg_write(tfa, 0x5b, 0x74e2); //POR=0x7329
		tfa_reg_write(tfa, 0x5c, 0x302b); //POR=0x5e96
		tfa_reg_write(tfa, 0x5f, 0x00a0); //POR=0x00c0
		tfa_reg_write(tfa, 0x62, 0x05c6); //POR=0x0582
		tfa_reg_write(tfa, 0x63, 0x80d4); //POR=0x0602
		tfa_reg_write(tfa, 0x67, 0x0066); //POR=0x0602
		tfa_reg_write(tfa, 0x68, 0x0820); //POR=0x0c20
		tfa_reg_write(tfa, 0x74, 0x60f0); //POR=0x4cf0
		tfa_reg_write(tfa, 0x75, 0x0e00); //POR=0x1200
		tfa_reg_write(tfa, 0x76, 0xbf00); //POR=0xf000
	...
	case 0x100b64:/**TFA9864 N2B0**/
		...
		tfa_reg_write(tfa, 0x5c, 0xb02b); //POR=0xde96
		tfa_reg_write(tfa, 0x5f, 0x00a0); //POR=0x00c0
		tfa_reg_write(tfa, 0x62, 0x06c4); //POR=0x0682
		tfa_reg_write(tfa, 0x63, 0x80d4); //POR=0x0602
		tfa_reg_write(tfa, 0x65, 0x0c58); //POR=0x0458
		tfa_reg_write(tfa, 0x67, 0x006e); //POR=0x0602
		tfa_reg_write(tfa, 0x68, 0x0820); //POR=0x0c20
		tfa_reg_write(tfa, 0x69, 0x0119); //POR=0x0319
		tfa_reg_write(tfa, 0x74, 0x6028); //POR=0x4c14
		tfa_reg_write(tfa, 0x75, 0x1daa); //POR=0x49e0
		tfa_reg_write(tfa, 0x76, 0xbe00); //POR=0xf000
		tfa_reg_write(tfa, 0x78, 0x0001); //POR=0x000d
		tfa_reg_write(tfa, 0x7c, 0x10f2); //POR=0x1602
	...
	/* we come from reset state so turn off osc */
	tfa_set_bf(tfa, TFA986X_BF_MANAOOSC, 1);

	/* select error interrupts */
	irqmask = (TFA_BF_MSK(TFA986X_BF_IEBSTOC) |
				TFA_BF_MSK(TFA986X_BF_IEOTDS) |
				TFA_BF_MSK(TFA986X_BF_IEOCPR) |
				TFA_BF_MSK(TFA986X_BF_IEUVDS) |
				TFA_BF_MSK(TFA986X_BF_IEBODNOK)|
				TFA_BF_MSK(TFA986X_BF_IECOOR)|
				TFA_BF_MSK(TFA986X_BF_IEOVDS));
...
static int tfa986x_set_bitfield(struct tfa_device* tfa, uint16_t bitfield, uint16_t value)
{
	if (((bitfield >> 8) & 0xff) == 0x10 || ((bitfield >> 8) & 0xff) == 0x13)
		return tfa_set_bf_volatile(tfa, (uint16_t)bitfield, value);
	else
...
static int tfa986x_set_swprofile(struct tfa_device *tfa, unsigned short new_value)
{
	int active_value = tfa_dev_get_swprof(tfa);
	...
	/* Set the new value in the hw register */
	tfa_set_bf_volatile(tfa, TFA986X_BF_SWPROFIL, new_value);
...
{
	return tfa_get_bf(tfa, TFA986X_BF_SWPROFIL) - 1;
}

static int tfa986x_set_swvstep(struct tfa_device *tfa, unsigned short new_value)
{
	...
	/* Set the new value in the hw register */
	tfa_set_bf_volatile(tfa, TFA986X_BF_SWVSTEP, new_value);
...
{
	return tfa_get_bf(tfa, TFA986X_BF_SWVSTEP) - 1;
}

/* tfa98xx_dsp_system_stable
*  return: *ready = 1 when clocks are stable to allow DSP subsystem access
*/
static enum Tfa98xx_Error tfa986x_dsp_system_stable(struct tfa_device *tfa, int *ready)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;

	/* check CLKS: ready if set */
	*ready = tfa_get_bf(tfa, TFA986X_BF_CLKS) == 1;
...
void tfa986x_ops(struct tfa_device_ops *ops)
{
	...
	ops->set_mute = tfa_set_mute_nodsp;
	ops->tfa_set_bitfield = tfa986x_set_bitfield;
}
